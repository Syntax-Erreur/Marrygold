...
export default function GuestListTable({
  ...
  // Fetch tables for the current event
  const fetchTables = async () => {
    try {
      const eventQuery = query(
        collection(db, "events"),
        where("name", "==", eventName),
        where("userId", "==", guests[0]?.userId || "") // Assume userId is available from guests
      );
      ...
      const fetchedTables: Table[] = tablesSnapshot.docs.map((doc) => ({
        id: doc.id,
        name: doc.data().name,
  ...
  const handleAddGuestToEvent = async (guestId: string) => {
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : "Failed to add guest to event"
      );
    } finally {
  ...
  const pageItemIds = useMemo(
    () => new Set(pageItems.map((guest) => guest.id)),
    [pageItems]
  );
  ...
  const deleteSelectedGuests = async () => {
    ...
    try {
      ...
      toast.success(
        `${selectedGuests.size} guest${selectedGuests.size > 1 ? "s" : ""} deleted successfully`
      );
    ...
    <div className="">
      <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogTitle>
            Delete Guest{selectedGuests.size > 1 ? "s" : ""}
          </DialogTitle>
          <div className="py-4">
            <p className="text-sm text-gray-700">
              Are you sure you want to delete {selectedGuests.size} guest
              {selectedGuests.size > 1 ? "s" : ""}? This action cannot be
              undone.
            </p>
          </div>
              ...
              <TableRow className="border-none">
                ...
                <TableHead className="w-[50px] py-5 whitespace-nowrap font-medium tracking-[0.36px]">
                  <div className="flex items-center gap-1">
                    <span className="font-inter text-xs text-[#252525] uppercase">
                      #
                    </span>
                    <Image
                ...
                <TableHead className="py-5 text-center whitespace-nowrap">
                  <div className="flex items-center justify-center gap-1">
                    <span className="font-inter font-medium text-xs leading-[18px] text-[#252525] uppercase">
                      TABLE NO
                    </span>
                    <Image
                      src="https://res.cloudinary.com/dvfk4g3wh/image/upload/v1743310626/marrygold/column-sorting_rz0xzo.png"
                ...
                <TableHead className="py-5 text-center whitespace-nowrap">
                  <div className="flex items-center justify-center gap-1">
                    <span className="font-inter font-medium text-xs leading-[18px] tracking-[0.03em] text-[#252525] uppercase">
                      EVENTS
                    </span>
                    <Image
              ...
              {currentPageItems.map((guest, index) => (
                ...
                >
                  ...
                  <TableCell className="py-4 text-sm font-normal text-[#252525]">
                    <span className="font-inter font-medium text-xs leading-[18px] text-[#252525]">
                      {startIndex + index + 1}
                    </span>
                  </TableCell>
                    ...
                    <div className="flex justify-center">
                      {guest.foodPreference === "Veg" ? (
                        <div className="inline-flex items-center gap-1 bg-[#E1FCEF] rounded-lg px-5 py-2">
                          ...
                          <span className="font-inter font-medium text-xs leading-[18px] text-[#14804A]">
                            Veg
                          </span>
                        </div>
                      ) : (
                        <div className="inline-flex items-center gap-1 bg-[#FCF2E6] rounded-lg px-5 py-2">
                          ...
                          <span className="font-inter font-medium text-xs leading-[18px] text-[#AA5B00]">
                            Non Veg
                          </span>
                        </div>
                      )}
                  ...
                  <TableCell className="py-4">
                    <Button
                      className="bg-[#252525] hover:bg-[#252525]/90 h-[34px] rounded-lg flex items-center gap-1.5 text-xs font-semibold px-5"
                      onClick={() => handleAddGuestToEvent(guest.id)}
          ...
          <div className="flex items-center justify-between py-6 px-5">
            ...
            <div className="flex items-center gap-6">
              <div className="flex items-center gap-2">
                <span className="text-xs font-medium text-[#252525] tracking-[0.36px]">
                  Rows per page:
                </span>
                <Select
